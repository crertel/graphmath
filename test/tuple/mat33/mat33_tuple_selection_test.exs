defmodule Graphmath.Mat33.Tuple.Selection_Mat33 do
  ...
  test "diag( 1:1:9 ) returns {1,5,9}" do
    assert { 1, 5, 9 } == Graphmath.Mat33.Tuple.diag( {1,2,3,4,5,6,7,8,9} )
  end
  
  @tag :mat33
  @tag :at
  test "at( 1:1:9, i, j ) returns element at [i,j] " do
    assert 1 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 0, 0 )
    assert 2 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 0, 1 )
    assert 3 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 0, 2 )
    
    assert 4 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 1, 0 )
    assert 5 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 1, 1 )
    assert 6 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 1, 2 )
    
    assert 7 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 2, 0 )
    assert 8 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 2, 1 )
    assert 9 == Graphmath.Mat33.Tuple.at( {1,2,3,4,5,6,7,8,9}, 2, 2 )
  end
